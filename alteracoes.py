...
class AmazonClimate:
    ...
    def __init__(self, env, season=SeasonType.RAINY, vegetation_density=0.8):
        ...
        # Adicionar parâmetros de vegetação mais detalhados
        self.tree_density = 0.15  # árvores por m² (valor típico para floresta densa)
        self.avg_tree_height = 25.0  # altura média das árvores em metros
        self.forest_depth_factor = 0.8  # fator de profundidade da floresta (0-1)
    ...
    def get_attenuation_factor(self, distance=100, frequency=915):
        """
        Retorna um fator de atenuação para comunicação baseado nas condições climáticas e vegetação
        
        Args:
            distance: Distância em metros até o gateway
            frequency: Frequência em MHz
        """
        # Chuva atenua significativamente sinal RF em frequências mais altas
        rain_attenuation = self.rain_intensity * 0.3 if self.is_raining else 0
        
        # Alta umidade também afeta o sinal - mais pronunciado que no modelo anterior
        humidity_factor = max(0, (self.current_humidity - 70) / 30)  # 0 a 1 para umidade de 70% a 100%
        humidity_attenuation = humidity_factor * 8  # Até 8 dB de atenuação com 100% de umidade
        
        # Atenuação adicional devido à vegetação
        vegetation_attenuation = calculate_vegetation_attenuation(
            distance, 
            frequency, 
            self.tree_density, 
            self.avg_tree_height,
            self.forest_depth_factor
        )
        
        # Fatores sazonais
        seasonal_factor = 1.5 if self.season == SeasonType.RAINY else 1.0
        
        total_attenuation = (rain_attenuation + humidity_attenuation + vegetation_attenuation) * seasonal_factor
...
def calculate_ldplm_path_loss(distance_m, freq_mhz, ple, d0_m=1.0):
    """Calcula a perda de percurso usando o modelo Log-Distance."""
    if distance_m <= d0_m:
        return 0.0  # Ou uma perda de espaço livre para d0
    # Path loss at reference distance d0 (Free Space Path Loss)
    pl_d0 = 20 * math.log10(d0_m) + 20 * math.log10(freq_mhz) - 27.55  # FSPL para d0 em metros e freq em MHz
    path_loss_db = pl_d0 + 10 * ple * math.log10(distance_m / d0_m)
    return path_loss_db

def calculate_vegetation_attenuation(distance_m, freq_mhz, tree_density, avg_tree_height, depth_factor):
    """
    ...
    Args:
        ...
        tree_density: Densidade de árvores por m²
        avg_tree_height: Altura média das árvores em metros
        depth_factor: Fator de profundidade da floresta (0-1)
    
    Returns:
        Atenuação adicional em dB
    """
    # Converter MHz para GHz para compatibilidade com modelos comuns
    freq_ghz = freq_mhz / 1000.0
    
    # Estimar profundidade da vegetação com base na distância e fator de profundidade
    veg_depth_m = distance_m * depth_factor
    
    # Coeficientes para o modelo de atenuação por vegetação (ajustáveis para calibração)
    alpha = 0.5  # Perda por unidade de densidade (dB/m²)
    beta = 2.0   # Inclinação da curva de perda
    gamma = 0.2  # Perda por unidade de altura (dB/m)
    
    # Cálculo da atenuação devido à densidade das árvores
    A = alpha * tree_density * (1 - math.exp(-beta * tree_density)) * veg_depth_m
    
    # Cálculo da atenuação devido à altura das árvores
    B = gamma * avg_tree_height
    
    # Atenuação específica por metro para frequências UHF em floresta densa
    specific_att = 0.3 * freq_ghz * tree_density  # dB/m (aumenta com frequência e densidade)
    
    # Atenuação total
    vegetation_loss = A - B + (specific_att * veg_depth_m)
    
    return max(0, vegetation_loss)  # Garantir que não seja negativo
...
class LoRaDevice:
    ...
    def calculate_rssi(self):
        """Calcula o RSSI com base na distância e condições ambientais usando modelos mais realistas"""
        # Parâmetros do modelo de propagação
        freq = 915  # Frequência em MHz (915 MHz é comum em LoRaWAN na América)
        
        # Determinar PLE (Path Loss Exponent) com base no ambiente e clima
        # Para floresta amazônica, o PLE varia entre 2.5 e 3.8 conforme a referência
        base_ple = 2.7  # Valor base para áreas florestais
        
        # Ajuste de PLE baseado em condições climáticas e densidade de vegetação
        if self.climate:
            ple_humidity_factor = 0.3 * (self.climate.current_humidity / 100)  # 0 a 0.3
            ple_rain_factor = 0.4 * (self.climate.rain_intensity / 30) if self.climate.is_raining else 0  # 0 a 0.4
            ple_vegetation_factor = 0.4 * self.climate.vegetation_density  # 0 a 0.4
            
            path_loss_exponent = base_ple + ple_humidity_factor + ple_rain_factor + ple_vegetation_factor
            path_loss_exponent = min(3.8, max(2.5, path_loss_exponent))  # Limitar entre 2.5 e 3.8
        else:
            path_loss_exponent = base_ple
        
        # Sombreamento (efeitos de pequena escala) modelados como variável aleatória gaussiana
        # Desvio padrão maior em ambiente florestal (4-6 dB)
        shadowing = random.gauss(0, 5 if self.climate and self.climate.vegetation_density > 0.5 else 3)
        
        # RSSI a 1m (calculado com a equação de Friis para a potência isotrópica)
        rssi_at_ref = -30  # dBm
        
        # Calcular perda de percurso usando o modelo Log-Distance
        path_loss = calculate_ldplm_path_loss(self.distance, freq, path_loss_exponent)
        
        # RSSI base = potência de referência - perda de percurso + sombreamento
        rssi_base = rssi_at_ref - path_loss + shadowing
        
        # Adicionar efeito do SF (SFs mais altos têm melhor sensibilidade)
        rssi_sf_bonus = (self.config.sf - 7) * 2.5
        
        # Potência de transmissão (em relação ao padrão de 14 dBm)
        tx_power_factor = self.config.tp - 14  # Diferença da potência padrão
        
        # Atenuação devido às condições climáticas e vegetação
        if self.climate:
            weather_vegetation_attenuation = self.climate.get_attenuation_factor(
                distance=self.distance, 
                frequency=freq
            )
            rssi_base -= weather_vegetation_attenuation
        
        # RSSI final = RSSI base + bônus do SF + ajuste de potência
        final_rssi = rssi_base + rssi_sf_bonus + tx_power_factor
        
        return round(final_rssi, 1)
